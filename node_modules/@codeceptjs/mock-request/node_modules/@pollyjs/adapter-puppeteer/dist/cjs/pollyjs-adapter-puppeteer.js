...
const LISTENERS = symbol$1();

const PASSTHROUGH_PROMISES = symbol$1();
...
class PuppeteerAdapter extends Adapter {
  ...
  onConnect() {
    ...
    this.assert('A puppeteer page instance is required.', !!(page && typeof page === 'object'));
    this.attachToPageEvents(page);
  }

  onDisconnect() {
    this[LISTENERS].forEach((_, target) => this._callListenersWith('removeListener', target));
  }
    ...
    this[LISTENERS].set(page, {
      request: async request => {
        if (requestResourceTypes.includes(request.resourceType())) {
          ...
          const headers = request.headers(); // A CORS preflight request is a CORS request that checks to see
          // if the CORS protocol is understood.

          const isPreFlightReq = method === 'OPTIONS' && !!headers['origin'] && !!headers['access-control-request-method']; // Do not intercept requests with the Polly passthrough QP

          if (url.includes(PASSTHROUGH_REQ_ID_QP)) {
            const parsedUrl = new utils.URL(url, true); // If this is a polly passthrough request
            // Get the associated promise object for the request id and set it
            // on the request.

            if (!isPreFlightReq) {
              this._requestsMapping.passthroughs.set(request, this[PASSTHROUGH_PROMISES].get(parsedUrl.query[PASSTHROUGH_REQ_ID_QP]));
      ...
      requestfinished: request => {
        ...
        const {
          passthroughs,
          pollyRequests
        } = this._requestsMapping; // Resolve the passthrough promise with the response if it exists
      ...
      requestfailed: request => {
        ...
        const {
          passthroughs,
          pollyRequests
        } = this._requestsMapping; // Reject the passthrough promise with the error object if it exists
  ...
  onRequest(pollyRequest) {
    ...
    const {
      promise
    } = pollyRequest; // Override the deferred promise's resolve and reject to no-op since
  ...
  async passthroughRequest(pollyRequest) {
    ...
    const parsedUrl = new utils.URL(url, true);
    parsedUrl.query[PASSTHROUGH_REQ_ID_QP] = requestId;

    try {
      const response = await new promise$1((resolve, reject) => {
        this[PASSTHROUGH_PROMISES].set(requestId, {
          resolve,
          reject
